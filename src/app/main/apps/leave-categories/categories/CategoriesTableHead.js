...
import { removeCategoryLeave } from "../store/categorySlice";
...
function CategoriesTableHead(props) {
  ...
  const { selectedCategoriesLeaveIds } = props;
  const numSelected = selectedCategoriesLeaveIds.length;

  const [selectedOrdersMenu, setSelectedOrdersMenu] = useState(null);

  const dispatch = useDispatch();
  
  const handleDeleteCategoryMessageClick = () => {
    enqueueSnackbar(
            ...
            >
              ...
              >
                <MenuList>
                  <MenuItem
                    onClick={(ev) => {
                      dispatch(removeCategoryLeave(selectedCategoriesLeaveIds));
                      props.onMenuItemClick();
                      closeSelectedOrdersMenu();
                      handleDeleteCategoryMessageClick(ev);
                    }}
                  >
