...
import { selectPendingLeaves, getLeaves } from "../../store/pendingLeavesSlice";
...
function PendingLeavesTable(props) {
  const classes = useStyles();
  const dispatch = useDispatch();

  const leaves = useSelector(selectPendingLeaves);
  const searchText = useSelector(
    ({ leavesApp }) => leavesApp.pendingLeaves.searchText
  );
...
export default withRouter(PendingLeavesTable);
